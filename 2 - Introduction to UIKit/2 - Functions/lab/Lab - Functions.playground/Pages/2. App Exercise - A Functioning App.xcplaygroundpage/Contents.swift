 ...
 */
var steps = 0
func incrementSteps(_ value: Int){
    steps += 1
    print(steps)
}
...
let t_percent = (goal * 10 )/100
let f_percent = (goal * 50)/100
let n_percent = (goal * 90)/100

func progressUpdate(steps: Int){
    if(steps < t_percent ){
        print("You're off to a good start.")
    }
    else if(steps < f_percent){
        print("You're almost halfway there!.")
    }
    else if(steps >= f_percent && steps < n_percent){
        print("You're are over halfway there!.")
    }
    else if(steps > n_percent && steps < goal){
