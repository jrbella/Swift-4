...
/* var result = nameInCaps == name ? "These two strings are equal" : "The two strings are not equal."
print(result)
 */
/*:
 ...
 */
//var result = nameInCaps.lowercased() == name.lowercased() ? "\(nameInCaps.lowercased()) and \(name) are the same" : "\(nameInCaps.lowercased()) and \(name.lowercased()) are not the same"
//print(result)
...
var result = (textToSearchThrough.lowercased()).contains(textToSearchFor.lowercased())  ? " I found it!!" : "False"
print(result)
